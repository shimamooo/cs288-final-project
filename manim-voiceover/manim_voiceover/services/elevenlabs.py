    ...
    def __init__(
        ...
        **kwargs,
        ...
        if voice_name:
            ...
            selected_voice = next(
                (v for v in available_voices if v.voice_id == voice_id), None
            )
        ...
        if language_code:
            if model not in ["eleven_turbo_v2_5", "eleven_flash_v2_5"]:
                raise Exception(
                    f"Language code {language_code} is not supported for model {model}. Needs model to be one of ['eleven_turbo_v2_5', 'eleven_flash_v2_5']"
                )
        ...
        **kwargs,
        ...
        if text_id is not None:
            assert isinstance(text_id, str), (
                f"text_id must be a string, got {type(text_id)}"
            )
        ...
        if text_id is not None:
            if text_id in self.consecutive_text_by_id:
                ...
                self.consecutive_text_by_id[text_id] = (
                    self.consecutive_text_by_id[text_id].rstrip() + " " + input_text
                ).rstrip() + " "
            else:
        ...
        final_enable_logging = (
            enable_logging if enable_logging is not None else self.enable_logging
        )
        final_optimize_streaming_latency = (
            optimize_streaming_latency
            if optimize_streaming_latency is not None
            else self.optimize_streaming_latency
        )
        final_language_code = language_code or self.language_code
        final_apply_text_normalization = (
            apply_text_normalization or self.apply_text_normalization
        )
        final_apply_language_text_normalization = (
            apply_language_text_normalization
            if apply_language_text_normalization is not None
            else self.apply_language_text_normalization
        )
        ...
        input_data = {
            ...
            "config": {
                ...
                "voice_settings": final_voice_settings.model_dump()
                if final_voice_settings
                else None,
